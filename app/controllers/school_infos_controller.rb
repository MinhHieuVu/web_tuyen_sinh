class SchoolInfosController < ApplicationController
  before_action :set_school_info, only: %i[ show edit update destroy ]
  ...
  def create
    ...
    respond_to do |format|
      if @school_info.save
        format.html { redirect_to school_info_url(@school_info), notice: "School info was successfully created." }
  ...
  def update
    respond_to do |format|
      if @school_info.update(school_info_params)
        format.html { redirect_to school_info_url(@school_info), notice: "School info was successfully updated." }
